...
const AddPostButton = ({ icon, clickHandler, children, ...props }) => {
  const {
    breakpoint, right, responsive, height, onGoToAddPost
  } = props
  const iconElement = <StyledIcon height={height ? height / 2.5 : undefined} icon={icon} />
  return (
    <StyledButton hasText={!!children} {...props} onClick={() => {console.log("click on add button"); clickHandler()}}>
      <Wrapper>
        {right || iconElement}
        {children &&
